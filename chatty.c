...
/**
 * Bitmap per mascherare i segnali dei worker
 */
...
void signal_handler_thread() {
    // Maschera SIGUSR2, che deve essere gestito dal listener
    sigset_t sigset;
    // Questo if dovrebbe eseguire perror appena dopo la chiamata errata perché
    // la condizione valuta gli operandi dell'OR in sequenza e appena uno
    // ritorna true passa ad eseguire il corpo dell'if
    if (sigemptyset(&sigset) < 0 || sigaddset(&sigset, SIGUSR2) < 0) {
        perror("creando la bitmap per mascherare i segnali nel signal handler");
        exit(EXIT_FAILURE);
    }
    if (pthread_sigmask(SIG_BLOCK, &sigset, NULL) == -1) {
        perror("pthread_sigmask nel signal handler");
        exit(EXIT_FAILURE);
    }
    while(1);
...
void* listener_thread(void* arg) {
    ...
    // Maschera i segnali che devono essere gestiti dal signal handler
    sigset_t sigset;
    // Questo if dovrebbe eseguire perror appena dopo la chiamata errata perché
    // la condizione valuta gli operandi dell'OR in sequenza e appena uno
    // ritorna true passa ad eseguire il corpo dell'if
    if (sigemptyset(&sigset) < 0
        ...
        perror("creando la bitmap per mascherare i segnali nel listener");
        exit(EXIT_FAILURE);
    }
    if (pthread_sigmask(SIG_BLOCK, &sigset, NULL) == -1) {
...
void* worker_thread(void* arg) {
    ...
    if (pthread_sigmask(SIG_BLOCK, &worker_signalmask, NULL) == -1) {
        perror("pthread_sigmask in un worker");
        exit(EXIT_FAILURE);
    }
...
int main(int argc, char *argv[]) {
    ...
    // Crea la bitmap dei worker per mascherare i segnali
    // Maschera i segnali che devono essere gestiti del signal handler
    // Maschera SIGUSR2, che deve essere gestito dal listener
    // Questo if dovrebbe eseguire perror appena dopo la chiamata errata perché
    // la condizione valuta gli operandi dell'OR in sequenza e appena uno
    // ritorna true passa ad eseguire il corpo dell'if
    if (sigemptyset(&worker_signalmask) < 0
        ...
        perror("creando la bitmap per mascherare i segnali in un worker");
        exit(EXIT_FAILURE);
    }
    ...
    freefd_ack = malloc(ThreadsInPool * sizeof(char));
